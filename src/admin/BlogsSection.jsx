...
const BlogsView = () => {
  ...
  const { loading, blogsAllData } = useSelector((state) => state.fetchallblogs);

  const handleOnClick = (id) => {
    navigate("/blogscontent", { state: id });
  };
          ...
          blogsAllData.map((blogsData, index) => (
            <Grid items key={Math.random() + index}>
              <Card elevation={0} className={cn(classes.innerGridWrapper)}>
                ...
                <div
                  className={cn(
                    classes.blogImageWrapper,
                    classes.paddingBottomImage
                  )}
                >
                  <img
                    src={blogsData.featuredimg}
                    className={cn(classes.blogImage)}
                  />
                </div>
                <Typography
                  variant="h6"
                  className={cn(classes.paddingBottomTypo)}
                >
                  {blogsData.catagories}
                </Typography>
                <StyledButton
                  onClick={() => {
                    handleOnClick(blogsData._id);
                  }}
                >
                  View Post
                </StyledButton>
                <NavLink
                  to={`/editBlogs/${blogsData._id}`}
                  state={{
                    title: blogsData.title,
                    catagories: blogsData.catagories,
                    blogsData: blogsData.blogspost,
                  }}
                >
                  <IconButton>
                    <EditIcon />
                ...
                <IconButton
                  onClick={(e) => {
                    handleBlogDelete(blogsData._id);
                  }}
                >
                  <DeleteIcon />
                </IconButton>
